"""
Legacy trade execution module.

The active repository workflow is scanner-only and does not call this module
from the main runtime path. Keep changes here isolated unless you are
explicitly working on order execution or trade-history compatibility.
"""
...
from src.internal.telegram_notifier import send_telegram_message
...
class TradeOrchestrator:
    ...
    def _save_trade_history(self, trade_record: Dict[str, Any]):
        ...
        """
        try:
            append_trade_history(self.trade_history_path, trade_record)
    ...
    def get_trade_pnl_summary(self) -> Dict[str, Any]:
        ...
        """
        try:
            return summarize_trade_history(self.trade_history_path)
        except Exception as e:
